...
    );
    
    // --- START: Favorite Course Prompt Notification ---
    try {
        ...
        if (course) {
             await sendNotification({
                ...
                sender: null,
                type: 'FAVORITE_COURSE_PROMPT',
                title: `You have great taste!`,
                message: `You favorited '${course.title}'. Turn your interest into a skill by enrolling today!`,
                data: { courseId: req.params.courseId }
            });
        }
    } catch(e) {
        console.error("Error sending favorite course notification:", e);
    }
    // --- END: Favorite Course Prompt ---
...
    );
    
    // --- START: Delayed Cart Reminder Notification ---
    setTimeout(async () => {
        try {
            const user = await User.findById(req.user.id);
            const course = await Course.findById(req.params.courseId);
            if (user && course && user.cart.includes(req.params.courseId)) {
                await sendNotification({
                    recipients: [req.user.id],
                    sender: null,
                    type: 'CART_REMINDER',
                    title: "Don't Forget Your Course!",
                    message: `The course '${course.title}' is waiting for you in your cart. Complete your purchase now!`,
                    data: { courseId: req.params.courseId }
                });
            }
        } catch (e) {
            console.error("Error sending delayed cart notification:", e);
        }
...
            if (admins.length > 0) {
                ...
                console.log("3. Admin IDs to notify:", adminIds);

            console.log("4. Calling sendNotification for admins...");
                await sendNotification({
                    recipients: adminIds,
                    sender: user._id,
                    type: 'NEW_STUDENT_REGISTERED',
                    title: 'New Student Created',
                    message: `An admin has created a new student profile for ${user.name}.`,
                    data: { studentId: user._id.toString() }
                });
                console.log("5. sendNotification for admins finished.");
            }
        }
    } catch (notificationError) {
        console.error('Error sending new student notification:', notificationError);
...
        });
        await newUser.save();
        console.log(`✅ New student '${newUser.name}' saved to database.`);
        
        // --- START: Notification Logic ---
        try {
            // Find all admins
            console.log("1. Starting notification logic...");
            const admins = await User.find({ role: 'admin' }).select('_id');
            console.log(`2. Found ${admins.length} admin(s).`);
            const adminIds = admins.map(admin => admin._id);
            ...
            if (adminIds.length > 0) {
                await sendNotification({
                    ...
                    message: `A new student, ${newUser.name}, has just signed up.`,
                    data: { studentId: newUser._id.toString() }
                });
            }
            ...
            if (instructorIds.length > 0) {
                await sendNotification({
                    ...
                    message: `A new student, ${newUser.name}, has joined MiSkills.`,
                    data: { studentId: newUser._id.toString() }
                });
            }
            
            // 3. ✅ Send Welcome Notification to the new student
            await sendNotification({
                recipients: [newUser._id],
                sender: null, // From the system
                type: 'WELCOME_STUDENT',
                title: 'Welcome to MiSkills!',
                message: `Hi ${newUser.name}, we're thrilled to have you join our learning community!`,
            });
