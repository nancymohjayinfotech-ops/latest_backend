...
// ADDED: Import the new notification service
...
        await event.save();
        ...
        // --- START: Notification Logic for Event Enrollment ---
        try {
            const eventOrganizerId = event.createdBy;

            // 1. Notify the Event Organizer of the new enrollment request
            if (eventOrganizerId) {
                await sendNotification({
                    recipients: [eventOrganizerId],
                    sender: studentId,
                    type: 'NEW_EVENT_ENROLLMENT',
                    title: 'New Enrollment Request',
                    message: `${studentDetails.name} has requested to enroll in your event '${event.title}'.`,
                    data: { eventId: event._id.toString(), userId: studentId }
                });
            }

            // 2. Send a confirmation to the user who enrolled
            await sendNotification({
                recipients: [studentId],
                sender: eventOrganizerId,
                type: 'event_enrollment_approved', // Or a new 'EVENT_ENROLLMENT_PENDING' type
                title: 'Enrollment Request Received',
                message: `Your request to enroll in '${event.title}' has been received. You will be notified of your status soon.`,
                data: { eventId: event._id.toString() }
            });

        } catch(e) {
            console.error("Error sending event enrollment notification:", e);
        }
